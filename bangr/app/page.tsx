...
import { Search, TrendingUp, MessageCircle, Repeat2, Heart, Eye, Plus, ExternalLink, User, LogOut, Copy, ChevronDown } from "lucide-react";
...
function BangrCard({
  ...
  return (
    <div className={`${color} rounded-3xl border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] hover:translate-y-[-2px] transition-all duration-200 p-4 relative`}>
      {/* View Market Button - Top Right */}
      <Link href={`/market/${id}`}>
        <button className="absolute top-4 right-4 px-3 py-2 bg-white text-black rounded-xl border-3 border-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[1px] hover:translate-y-[1px] active:shadow-none active:translate-x-[3px] active:translate-y-[3px] transition-all flex items-center gap-1.5 z-10" style={{ fontWeight: 700 }}>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="px-5 py-3 bg-white text-black rounded-xl border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] active:shadow-none active:translate-x-[4px] active:translate-y-[4px] transition-all flex items-center gap-2"
                ...
                <>
                  ...
                  <div className="relative" ref={dropdownRef}>
                    <button
                      ...
                      className="px-4 py-3 bg-white text-black rounded-xl border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[1px] hover:translate-y-[1px] transition-all flex items-center gap-2"
                      style={{ fontWeight: 700 }}
                    >
                      {address.slice(0, 6)}...{address.slice(-4)}
                      <ChevronDown className={`w-4 h-4 transition-transform ${walletDropdownOpen ? 'rotate-180' : ''}`} strokeWidth={3} />
                    </button>

                    {walletDropdownOpen && (
                      <div className="absolute right-0 mt-2 w-56 bg-white border-4 border-black rounded-xl shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] overflow-hidden z-50">
                        <div className="p-3 border-b-3 border-black bg-yellow-100">
                          <p className="text-xs text-gray-600" style={{ fontWeight: 600 }}>Connected Wallet</p>
                          <p className="text-sm font-mono mt-1" style={{ fontWeight: 700 }}>{address.slice(0, 10)}...{address.slice(-8)}</p>
                        </div>
                        <button
                          onClick={copyAddress}
                          className="w-full px-4 py-3 text-left hover:bg-yellow-50 transition-colors flex items-center gap-3 border-b-3 border-black"
